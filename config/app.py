...
def login_required(role=None):
    """
    Decorator that enforces an active Flask session.
    Optionally restrict to a specific role: 'lecturer' or 'student'.

    Usage:
        @login_required()                   # any authenticated user
        @login_required(role='lecturer')    # lecturers only
        @login_required(role='student')     # students only
    """
    def decorator(f):
        @functools.wraps(f)
...
def signup_api():
    """
    Register a new lecturer or student account.
    ...
    Expected JSON body:
        {
            ...
            "identifier": "tutor_id (lecturer) or reg_no (student)"
        }
    """
    ...
    if not all([email, password, role, identifier]):
        return jsonify({'error': 'email, password, role, and identifier are all required.'}), 400

    result = ledger.users.register(email, password, role, identifier)
    if not result['success']:
        return jsonify({'error': result['error']}), 409   # 409 Conflict for duplicate email

    # Auto sign-in after registration — store user in session
...
def signin_api():
    """
    Authenticate an existing user and open a Flask session.
    ...
    result = ledger.users.authenticate(email, password)
    if not result['success']:
...
def me():
    """Return the currently signed-in user, or 401 if not authenticated."""
    user = session.get('user')
    if not user:
...
@app.route('/acct/signup')
def signup():
    return app.send_static_file('acct/signup.html')
    
@app.route('/acct/signin')
def signin():
